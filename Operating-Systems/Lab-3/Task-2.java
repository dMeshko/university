...
public class TemplateNumRunsAndNumInstances {
	 ...
	 *
	 */
	public static Semaphore isAwake;
	public static Semaphore waiting;
	public static int noWaiting;
	public static ReentrantLock lock;
	public static Semaphore canSit;
	public static Semaphore ready;

	public static void init(int numBarbers) {
		isAwake = new Semaphore(0);
		waiting = new Semaphore(5);
		noWaiting = 0;
		lock = new ReentrantLock();
		canSit = new Semaphore(0);
	...
	static class State {
		...
		public synchronized void barberWakeUp() throws RuntimeException {
			...
			if (barberWaked[b.barberId]) {
				PointsException e = new PointsException(5,
						"Berberot e veke buden i nema potreba da se razbudi.");
				log(e, null);
			} else {
		...
		public synchronized void barberCallCustomer() throws RuntimeException {
			...
			if (arrivedCustomers <= 0) {
				PointsException e = new PointsException(5,
						"Brojot na klienti koi cekaat e 0 i nema koj da bide povikan.");
				log(e, null);
			}
			calledCustomers++;
		}
		...
		public synchronized void customerEntry() throws RuntimeException {
			...
			if (arrivedCustomers <= 0) {
				PointsException e = new PointsException(5,
						"Brojot na klienti koi cekaat e 0 i nema koj da vleze.");
				log(e, null);
			}
			if (calledCustomers <= 0) {
				PointsException e = new PointsException(5,
						"Nema povikano klient i ne moze da vleze.");
				log(e, null);
			}
			arrivedCustomers--;
			calledCustomers--;
		...
		public void cutHair() throws RuntimeException {
			synchronized (this) {
				if (numCuttings <= 0) {
					PointsException e = new PointsException(5,
							"Nema prisuten klient za potstrizuvanje");
					log(e, null);
				}
			...
			synchronized (this) {
				if (numCuttings <= 0) {
					PointsException e = new PointsException(5,
							"Brojot na klienti koi se strizat e 0 i nema koj da izleze.");
					log(e, null);
				}
				numCuttings--;
			}
		...
		public synchronized void barberGoToSleep() throws RuntimeException {
			...
			if (!barberWaked[b.barberId]) {
				PointsException e = new PointsException(5,
						"Berberite veke spijat i ne moze da se prezaspijat.");
				log(e, null);
			}
			if (arrivedCustomers > 0) {
				PointsException e = new PointsException(5,
						"Seuste ima klienti koi cekaat i berberot ne moze da odi na spienje.");
				log(e, null);
			}
		...
		public void printStatus() {
			if (!TemplateThread.hasException) {
				...
				System.out
						.println("====================================================");
				PointsException.printErrors();
				System.out.println("Maksimum Poeni: "
						+ (25 - PointsException.getTotalPoints()));
			}
